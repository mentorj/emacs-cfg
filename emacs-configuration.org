...
* Looks
** Xclip
Enables integration with  X clipboard from Emacs
...
** All the icons 
Use the nice package all-the-icons
...
** Modeline
Use the doom-modeline , very pleasant & useful
#+BEGIN_SRC emacs-lisp
...
** Disable ansi color in compilation mode
   ...
   #+end_src
* Evil
Evil makes Emacs more Vi like with consistent keybindings.
...
** Magit
Best frontend for Git  is Magit of course..
#+BEGIN_SRC emacs-lisp
(use-package magit
  :ensure t
...
** Beacon
Visual hint for the cursor when switching buffers
...
** Recentf
ACtivates the recent files mode
#+BEGIN_SRC emacs-lisp
(require 'recentf)
(recentf-mode 1)
(global-set-key "\M-o" 'counsel-recentf)
...
** Plantuml
Nice tool to draw UML diagrams.Install it now...
...
** Graphviz
Required to draw graph like diagrams
...
** Vterm
VTerm is a beautiful, fast & robust terminal. Requires C compilation at install.Done automatically if the machine has the right dependencies.
